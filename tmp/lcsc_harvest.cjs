/* Resumable LCSC harvester: walks tmp/lcsc_queue.json, saves ranked rows to
   tmp/lcsc_results.jsonl. Usage:
...
const queue = JSON.parse(fs.readFileSync(path.join(ROOT, 'tmp/lcsc_queue.json'), 'utf-8'));
const outPath = path.join(ROOT, 'tmp/lcsc_results.jsonl');
const done = new Set();
